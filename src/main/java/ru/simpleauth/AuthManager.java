...
public class AuthManager {
    ...
    private long lastReminderCheck = 0L;
    private final Map<String, String> firedReminders = new ConcurrentHashMap<>();
    private final Map<UUID, Integer> lastSlot = new ConcurrentHashMap<>();
    private final Map<UUID, Long> clockMuted = new ConcurrentHashMap<>();
    ...
    public void onDisconnect(ServerPlayerEntity player) {
        ...
        lastSlot.remove(player.getUuid());
        clockMuted.remove(player.getUuid());
    ...
    public void tick(MinecraftServer server) {
        ...
        tickClock(server, now);
        tickReminders(server, now);
    ...
    private ZoneId zone() {
        try {
            return ZoneId.of(config.clock.timeZone);
        } catch (Exception e) {
            return ZoneId.systemDefault();
        }
    }

    private void tickClock(MinecraftServer server, long now) {
        if (!config.clock.enabled) return;
        if (now - lastClock < Math.max(50, config.clock.updateMs)) return;
        lastClock = now;

        ZonedDateTime time = ZonedDateTime.now(zone());
        String line = "\u00AB " + time.format(DateTimeFormatter.ofPattern("HH:mm:ss")) + " \u00BB";
        if (config.clock.showDate) {
            line += "  " + time.format(
                    DateTimeFormatter.ofPattern("EEE, d MMM", Locale.forLanguageTag("ru")));
        }

        Text text = config.clock.shimmer
                ? shimmer(line, shimmerPhase * 0.45F, 0.42F, 0.018F)
                : Text.literal(line).formatted(Formatting.AQUA);

        for (ServerPlayerEntity player : server.getPlayerManager().getPlayerList()) {
            // у не вошедших над хотбаром свой отсчёт — не перебиваем
            if (!isAuthenticated(player)) continue;

            // Название предмета выводится в ту же строку, что и часы.
            // Поэтому при смене слота молчим пару секунд, иначе название
            // предмета вообще не увидеть.
            UUID uuid = player.getUuid();
            int slot = player.getInventory().selectedSlot;
            Integer previous = lastSlot.put(uuid, slot);
            if (previous != null && previous != slot) {
                clockMuted.put(uuid, now + 2000L);
            }
            Long mutedUntil = clockMuted.get(uuid);
            if (mutedUntil != null && now < mutedUntil) continue;

            player.sendMessage(text, true);
        }
    }

    private void tickReminders(MinecraftServer server, long now) {
        if (config.reminders == null || config.reminders.isEmpty()) return;
        if (now - lastReminderCheck < 1000L) return;
        ...
        for (Config.Reminder reminder : config.reminders) {
            if (reminder == null || !reminder.enabled || reminder.time == null) continue;

            LocalTime target;
            try {
                target = LocalTime.parse(reminder.time.trim());
            } catch (Exception e) {
                continue;
            }

            ServerPlayerEntity player = findOnline(server, reminder.player);
            if (player == null || !isAuthenticated(player)) continue;

            if (current.equals(target)) {
                if (markFired(reminder, "now", today)) {
                    fireReminder(player, reminder);
                }
            } else if (reminder.warnMinutesBefore > 0
                    && current.equals(target.minusMinutes(reminder.warnMinutesBefore))) {
                if (markFired(reminder, "warn", today)) {
                    send(player, String.format(config.messages.reminderWarn,
                            reminder.warnMinutesBefore, reminder.text), Formatting.GOLD);
                    if (config.playSounds) {
                        sound(player, SoundEvents.BLOCK_NOTE_BLOCK_BELL, 1.4F);
                    }
                }
            }
        }

        // чистим отметки прошлых дней, чтобы карта не росла
        if (firedReminders.size() > 64) {
            firedReminders.entrySet().removeIf(e -> !today.equals(e.getValue()));
        }
    }

    private boolean markFired(Config.Reminder reminder, String kind, String today) {
        String key = reminder.player + "|" + reminder.time + "|" + kind;
        return !today.equals(firedReminders.put(key, today));
    }

    private void fireReminder(ServerPlayerEntity player, Config.Reminder reminder) {
        if (config.showTitles) {
            showTitle(player,
                    Text.literal(reminder.title).formatted(Formatting.GOLD, Formatting.BOLD),
                    Text.literal(reminder.text).formatted(Formatting.WHITE),
                    5, 70, 15);
        }
        send(player, String.format(config.messages.reminderNow, reminder.text), Formatting.GOLD);
        if (config.playSounds) {
            sound(player, SoundEvents.BLOCK_NOTE_BLOCK_BELL, 1.0F);
            sound(player, SoundEvents.BLOCK_NOTE_BLOCK_BELL, 1.5F);
        }
        SimpleAuth.LOGGER.info("[SimpleAuth] Напоминание {} для {}", reminder.time, reminder.player);
    }

    private static ServerPlayerEntity findOnline(MinecraftServer server, String name) {
