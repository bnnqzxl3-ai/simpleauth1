...
public class AuthManager {
    ...
    /** Префикс игрока или null, если его нет. Ник сверяется без учёта регистра. */
    public Text prefixFor(String name) {
        if (config.prefixes == null || config.prefixes.isEmpty()) return null;
        String symbol = null;
        for (Map.Entry<String, String> entry : config.prefixes.entrySet()) {
        ...
        Formatting color = Formatting.byName(config.prefixColor);
        MutableText text = Text.literal(symbol);
        return color != null ? text.formatted(color) : text;
    }

    /** Ставит префикс в список игроков (Tab). */
    public void applyTabPrefix(ServerPlayerEntity player) {
        String name = player.getGameProfile().getName();
        Text prefix = prefixFor(name);
        if (prefix == null) {
            player.setPlayerListName(null);
            return;
        }
        player.setPlayerListName(Text.empty()
                .append(prefix)
